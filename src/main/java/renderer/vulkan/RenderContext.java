...
public class RenderContext {
    ...
    private static boolean isPhysicalDeviceSuitable(MemoryStack stack, VkPhysicalDevice physicalDevice) {
        ...
        Utils.check(vkEnumerateDeviceExtensionProperties(physicalDevice, (String)null, ip, null));
        if (ip.get(0) > 0) {
            // Get available extensions
            VkExtensionProperties.Buffer deviceExtensions = VkExtensionProperties.mallocStack(ip.get(0), stack);
            Utils.check(vkEnumerateDeviceExtensionProperties(physicalDevice, (String)null, ip, deviceExtensions));
    ...
    private void pickPhysicalDevice() {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            Utils.check(vkEnumeratePhysicalDevices(this.instance, ip, null));
            if (ip.get(0) > 0) {
                PointerBuffer physicalDevices = stack.mallocPointer(ip.get(0));
                Utils.check(vkEnumeratePhysicalDevices(this.instance, ip, physicalDevices));
    ...
    private void createDevice() {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            VkDeviceQueueCreateInfo.Buffer queueCreateInfos = VkDeviceQueueCreateInfo.mallocStack(2, stack);

            queueCreateInfos.position(0);
            queueCreateInfos.sType(VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO);
            queueCreateInfos.pNext(NULL);
            queueCreateInfos.flags(0);
            queueCreateInfos.queueFamilyIndex(this.graphicsQueueFamilyIndex);
            queueCreateInfos.pQueuePriorities(stack.floats(0.0f));

            queueCreateInfos.position(1);
            queueCreateInfos.sType(VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO);
            queueCreateInfos.pNext(NULL);
            queueCreateInfos.flags(0);
            queueCreateInfos.queueFamilyIndex(this.presentQueueFamilyIndex);
            queueCreateInfos.pQueuePriorities(stack.floats(0.0f));
            ...
            Utils.check(vkCreateDevice(this.physicalDevice, createInfo, null, pp));
    ...
    public RenderContext(WindowContext windowContext) {
        this.windowContext = windowContext;

        createInstance();
        this.surface = this.windowContext.createWindowSurfaceWSI(this.instance);
